...
Phase 4: [Client] → [Server] → [Redis Counter + SET] → [MySQL]  ← 현재
         └─ 개선: 두 단계 Fast Fail, 응답시간 34.5% 개선
         └─ 핵심: DB 접근 전 Redis 검증 완료
```
...
## 7. Phase 4: 최적화된 Redis 기반 처리 (Optimized)

### 7.1 목표

**진정한 Fast Fail 구현**: Redis 체크를 먼저 수행하여 불필요한 DB 접근을 최소화합니다.

### 7.2 아키텍처

```
[Client] → [Server] → [Redis Counter] → [Redis SET] → [MySQL]
                            ↓                ↓
                      (Fast Fail 1)    (Fast Fail 2)
                      쿠폰 소진         중복 발급
```

### 7.3 Phase 3 대비 개선 포인트

| Phase 3 문제 | Phase 4 해결 방안 |
|:---|:---|
| DB 접근 후 Redis 체크 | Redis 체크 먼저, DB 접근 나중 |
| 중복 체크가 DB에서 수행 | Redis SET으로 중복 체크 |
| Fast Fail 효과 제한적 | 두 단계 Fast Fail 구현 |

### 7.4 구현 방식

Redis 카운터와 SET을 활용한 최적화된 처리입니다.
...
public class CouponService {
    ...
    public CouponIssueResponse issueWithOptimizedRateLimiting(Long userId) {
        ...
        if (currentCount > TOTAL_COUPON_COUNT) {
            counter.decrementAndGet();
            throw new CouponSoldOutException();  // Fast Fail 1
        }
        ...
        if (!isNewUser) {
            counter.decrementAndGet();
            throw new AlreadyIssuedException(userId);  // Fast Fail 2
        }
...
| **발급 사용자 SET** | `coupon:issued:users` | 발급받은 사용자 ID SET (중복 체크) |
...
### 7.8 Phase 3 vs Phase 4 성능 비교

```
[Phase 3 vs Phase 4 처리 흐름]

Phase 3:
[요청] → [사용자 검증 (DB)] → [중복 체크 (DB)] → [Redis Counter] → [쿠폰 발급]
              ↑ 병목                ↑ 병목

Phase 4:
[요청] → [Redis Counter] → [Redis SET] → [사용자 검증 (DB)] → [쿠폰 발급]
              ↓                 ↓               ↓
         Fast Fail 1       Fast Fail 2    유효한 요청만 도달
```
...
| 시나리오 | Phase 3 | Phase 4 |
|:---|:---|:---|
| 쿠폰 소진 후 요청 | DB 2회 접근 후 Redis 거절 | Redis 즉시 거절 (DB 0회) |
| 중복 발급 요청 | DB 2회 접근 | Redis 즉시 거절 (DB 0회) |
| 유효한 요청 | DB 3회 접근 | DB 2회 접근 |

### 7.9 Phase 4 결론

Redis 기반 두 단계 Fast Fail을 구현하여 **불필요한 DB 접근을 완전히 제거**했습니다.

**핵심 성과:**
1. **응답시간 34.5% 개선**: 20,811ms → 13,619ms
2. **최소 응답시간 75% 개선**: 7,057ms → 1,766ms
3. **두 단계 Fast Fail**: 수량 체크 + 중복 체크 모두 Redis에서 처리
4. **DB 접근 최소화**: 쿠폰 소진/중복 발급 요청은 DB 접근 없이 거절

**한계:**
- TPS는 VUser 감소(600→300) 및 DB 쓰기 병목으로 소폭 하락
- 근본적인 TPS 향상을 위해서는 비동기 처리(Kafka 등) 도입 필요
...
| Phase 3 → 4 | Redis 기반 중복 체크 | 응답시간 34.5% 개선, 최소 응답시간 75% 개선 |
...
| Redis SET 중복 체크 | DB 접근 없이 O(1) 중복 검증 |
